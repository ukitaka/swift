...
public:
  ...
  ProtocolDecl *makeProtocol(DeclContext *DC, StringRef name) {
    SmallVector<TypeLoc, 4> InheritedProtocols;
    auto result = new (Ctx) ProtocolDecl(DC, SourceLoc(), SourceLoc(), Ctx.getIdentifier(name), Ctx.AllocateCopy(InheritedProtocols), nullptr);
    result->setAccess(AccessLevel::Internal);
    return result;
  }
};
